...
export default class CardRepositoryDtabase implements CardRepository {
    ...
    async save(card: Card): Promise<number> {
        const [cardData] = await this.connection.query(
            "insert into j.card (id_column, title, estimative) values ($1, $2, $3) returning *",
    ...
    async update(card: Card): Promise<void> {
        await this.connection.query(
            "update j.card set title = $1, estimative = $2 where id_card = $3",
            [card.title, card.estimative, card.idCard]
        );
    }

    async delete(idCard: number): Promise<void> {
        await this.connection.query("delete from j.card where id_card = $1", [
            idCard,
        ]);
    }

    async updateIdColumnAndIndex(
        idCard: number,
        idColumn: number,
        index: number
    ): Promise<void> {
        await this.connection.query(
            "update j.card set id_column = $1, index = $2 where id_card = $3",
            [idColumn, index, idCard]
        );
    }

    list(title: string): Promise<Card[]> {
        if (!title) {
            return this.connection.query("select * from j.card", []);
        } else {
            return this.connection.query(
                "select * from j.card where title like $1",
                [`%${title}%`]
            );
        }
    }
    ...
    async get(idCard: number): Promise<Card> {
        const [cardData] = await this.connection.query(
            "select * from j.card where id_card = $1",
            [idCard]
        );
